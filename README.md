...
- **一百件事 · 三种关系** — 每件事属于「我为你做（父母→孩子）」「你为我做（孩子→父母）」或「我们一起做」之一；首页跨孩子混合呈现，每张卡片直接标明对象，并按时段、季节、周末和孩子年龄加权推荐。
...
DOOPUSH_OPPO_APP_KEY=...           # Android OEM 通道；仅构建期使用
DOOPUSH_OPPO_APP_SECRET=...
DOOPUSH_OPPO_APP_KEY_DEV=...       # 可选：Dev 变体 OPPO 凭据
DOOPUSH_OPPO_APP_SECRET_DEV=...
DOOPUSH_VIVO_APP_ID=...            # vivo 通道客户端凭据（AppID）；仅构建期使用
DOOPUSH_VIVO_API_KEY=...           # vivo 的 AppKey（AppSecret 只在 DooPush 后台配，不进包）
DOOPUSH_VIVO_APP_ID_DEV=...        # 可选：Dev 变体 vivo 凭据
DOOPUSH_VIVO_API_KEY_DEV=...
```
