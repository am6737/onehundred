...
export function GovernanceAccessCard({
  ...
  const reasonId = `governance-access-reason-${generatedId}`
      ...
      <Card className="border-amber-300/60 bg-amber-50/70 shadow-sm dark:border-amber-500/30 dark:bg-amber-500/8">
        <CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
          <div className="flex gap-3">
            <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-amber-500/15 text-amber-700 dark:text-amber-300"><LockKeyholeIcon className="size-5" aria-hidden="true" /></span>
          ...
          <Button onClick={onLoad} disabled={loading} className="shrink-0"><ShieldCheckIcon aria-hidden="true" />{loading ? "正在加载…" : loadedReason ? "刷新数据" : "加载数据"}</Button>
        </CardContent>
      </Card>
    )
  }
        ...
        <div className="flex gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
              {loadedReason ? <CheckCircle2Icon className="size-3.5 text-emerald-600" aria-hidden="true" /> : <ShieldCheckIcon className="size-3.5" aria-hidden="true" />}
              {loadedReason ? `已授权：${loadedReason}` : "访问会记录在治理与审计链路中"}
            </div>
          </div>
        </div>
        <div className="grid gap-2">
          <Label htmlFor={reasonId}>治理访问理由 <span className="text-destructive" aria-hidden="true">*</span></Label>
          <Textarea
            id={reasonId}
            name="governance-access-reason"
            autoComplete="off"
            value={reason}
            onChange={(event) => onReasonChange(event.target.value)}
            className={cn(
              "min-h-24 rounded-xl bg-background/90 px-3.5 py-3 shadow-xs",
              reason.length > 0 && !isValid && "border-amber-400/70",
            )}
            placeholder={placeholder.endsWith("…") ? placeholder : `${placeholder}…`}
            required
            minLength={minimumGovernanceReasonLength}
            aria-invalid={reason.length > 0 && !isValid}
            aria-describedby={`${reasonId}-status`}
          />
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <span id={`${reasonId}-status`} aria-live="polite" className={cn("text-xs text-muted-foreground", reason.length > 0 && !isValid && "text-amber-700 dark:text-amber-400")}>
