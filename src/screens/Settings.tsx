...
function IdentityRow({ me, options, onSelect, divider = false }) {
  ...
  return (
    <View>
      ...
      {/* Floating picker as a Modal */}
      <Modal transparent visible={open} animationType="fade" onRequestClose={() => setOpen(false)}>
        <Pressable style={StyleSheet.absoluteFill} onPress={() => setOpen(false)}>
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Pressable onPress={e => e.stopPropagation()}>
              <View style={{
                width: 200, backgroundColor: theme.paper,
                borderWidth: 1, borderColor: theme.line,
                borderRadius: 18, padding: 6,
                shadowColor: theme.ink, shadowOffset: { width: 0, height: 22 },
                shadowOpacity: 0.25, shadowRadius: 46, elevation: 10,
              }}>
                {options.map(o => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={{
                        flexDirection: 'row', alignItems: 'center',
                        gap: 10, paddingVertical: 11, paddingHorizontal: 12,
                        borderRadius: 12,
                        backgroundColor: on ? theme.sand : 'transparent',
                      }}
                    >
                      <Text style={{
                        flex: 1, fontFamily: theme.fonts.body, fontSize: 15.5,
                        color: on ? theme.accent : theme.ink,
                      }}>{o}</Text>
                      {on ? Icon.check(theme.accent, 16) : null}
                    </TouchableOpacity>
                  );
                })}
              </View>
...
export default function Settings({ navigation, route }) {
  ...
  // Route params: me and setMe
  const me = route?.params?.me || DEFAULT_ME;
  const setMe = route?.params?.setMe || (() => {});

  // Local state
  const [kids, setKids] = useState(() => KIDS.map(k => ({ ...k })));
  const [editId, setEditId] = useState(null);
  const [sheet, setSheet] = useState(null); // 'add'|'invite'|'remindTime'|'sealed'|'privacy'|'about'
      ...
      >
        ...
        <SettingGroup label="家庭">
          <Row
            ...
            title="家庭成员"
            sub={'爸爸、妈妈、' + kids.map(k => k.name).join('、')}
            value="邀请家人"
