...
- **按孩子区分**：给娃 A 记录过的事，选中娃 A 时隐藏，选中娃 B 时仍显示。注：在「全家」视图（kidId='all'）下记录时，`RecordFlow` 会把回忆归属到首位孩子（`kids[0]`），而非写入 `kid='all'`，因此一条回忆只对应一个孩子。过滤逻辑仍兼容 `kid='all'` 的历史记录（与现有 `kidDoneFrom` 计数口径一致），但正常写入路径不产生这类记录。
...
| 全家视图下记录 | 归属首位孩子（`kids[0]`），只对该孩子隐藏/回归——与「按孩子区分」一致。过滤逻辑仍兼容历史 `kid='all'` 记录，但写入路径不产生 |
...
- 数据库删除失败：详情页 Alert 提示「删除失败 / 没能删掉，稍后再试一次」，本地 state 不变。
