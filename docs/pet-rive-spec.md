...
做 3 个文件（bear / dog / cat），三者**共用同一套骨架与动画**，只换美术（见 0.1）：
...
## 0.1 母骨架 + 套皮（Duolingo 打法，强烈建议）

多邻国能撑起一整个角色阵容，靠的是「**一套骨架 + 一套动画库，跨角色复用，只换皮**」。
我们照搬，省下 ~2/3 工作量：

1. **先把 bear 做成「母文件」**：完整骨架 + 状态机 + 全部 9 情绪 + tap，调到满意。
2. **dog / cat 直接复制 bear 文件**，骨架、状态机、所有动画时间线**原样保留不动**，
   只把每根骨头上挂的矢量美术换成狗/猫的部件（reskin / 重绑皮）。
3. 于是「3 物种 × 9 情绪」的动画量 ≈ 只做 **1 份**。

> 前提：三个物种用**同一套部件划分与骨骼结构**（头/耳/眼/嘴/身/臂/腿/尾，骨头数量与命名一致）。
> 设计 bear 时就按「这套骨架以后要套狗和猫」来规划部件，别做只有熊才有的特殊结构。
