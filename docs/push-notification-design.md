...
5. **克制化重校准（产品方向修正）** —— 家庭记忆是低频行为，不照搬 Duolingo 每日打卡/内疚驱动，
   否则用户一打开就有「欠账」负罪感。故：①`FREQ_THRESHOLDS` 整体调稀（normal 首次温和提醒
   从 3 天 → **14 天**，升级阶梯 14/30/60；少 21/45/90；多 10/21/45）；②新增**提醒冷却**——
   任一提醒场景（gentle/growth/loss）在冷却期（=该档首提醒间隔）内发过就不再发提醒，取代原
   48h 去重，把「越过阈值后每 2 天念叨」改成「约每 2 周偶发一次」。正向惊喜场景（那年今天/
   里程碑/胶囊/连续/家人）不受冷却限制、仍是主角。
   （注：本轮暂不动 `loss_hint` 与升级语气本身——内疚文案的去留是独立决定，留待后续。）
